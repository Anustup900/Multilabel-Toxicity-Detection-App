...
category = {'LABEL_0': 'toxic', 'LABEL_1': 'severe_toxic', 'LABEL_2': 'obscene', 'LABEL_3': 'threat',
...
initial_table_df = pd.DataFrame(
    columns=["Text (portion)", "Toxicity class 1", "Class 1 probability", "Toxicity class 2", "Class 2 probability"])
    ...
    if not text:
        ...
        with st.spinner("Analyzing toxicity..."):
            if selected_model == "Olivernyu/finetuned_bert_base_uncased":
                ...
                st.session_state['table'] = pd.concat([pd.DataFrame(table_data), st.session_state['table']],
                                                      ignore_index=True)
